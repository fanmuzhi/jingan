...
uint32_t Synaptics_Site::GetTestResult(Synaptics_TestResult * &opSynapticsTestResult)
{
	...
	//serial number
	_pSynaptics_TestResult->strSensorNumber = pTestResult->_sSensorSerialNumber;
...
void Synaptics_Site::WriteLog(std::string SstrFolderPath, std::string strFileName)
{
	if (NULL != _pSyn_Site)
	{
		_pSyn_Site->WriteLog(SstrFolderPath, strFileName);
	}
}
