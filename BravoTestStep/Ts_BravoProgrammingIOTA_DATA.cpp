...
Ts_BravoProgrammingIOTA_DATA::Ts_BravoProgrammingIOTA_DATA(string &strName, FpBravoModule * &pSynModule, Syn_Dut_Utils * &pSynDutUtils)
:Syn_BravoFingerprintTest(strName, pSynModule, pSynDutUtils)
, _pProgrammingIOTA_Data_TestData(NULL)
{
}

Ts_BravoProgrammingIOTA_DATA::~Ts_BravoProgrammingIOTA_DATA()
{
}

void Ts_BravoProgrammingIOTA_DATA::SetUp()
{
	Syn_Exception Exception(0);
	if (NULL == _pSynModule)
	{
		Exception.SetError(ERROR_BRAVOMODULE_NULL);
		Exception.SetDescription("BravoProgrammingIOTA_DATA::_pSynModule is NULL!");
		throw Exception;
		return;
	}
	if (NULL == _pSynDutUtils)
	{
		...
		Exception.SetDescription("BravoProgrammingIOTA_DATA::_pSynDutUtils is NULL!");
		throw Exception;
		return;
	}
	...
	_pProgrammingIOTA_Data_TestData = new ProgrammingIOTA_Data_TestData();
	_pProgrammingIOTA_Data_TestData->data_name = _strName;
	_pProgrammingIOTA_Data_TestData->LNA_BL = true;
	_pProgrammingIOTA_Data_TestData->FW_BL = false;
	_pProgrammingIOTA_Data_TestData->NAV_BL = true;
	_pProgrammingIOTA_Data_TestData->WOF = true;
	...
	if (0 != listOfArgValue[0].length())
		_pProgrammingIOTA_Data_TestData->LNA_BL = atoi(listOfArgValue[0].c_str()) ? true : false;
	if (0 != listOfArgValue[1].length())
		_pProgrammingIOTA_Data_TestData->FW_BL = atoi(listOfArgValue[1].c_str()) ? true : false;
	if (0 != listOfArgValue[2].length())
		_pProgrammingIOTA_Data_TestData->NAV_BL = atoi(listOfArgValue[2].c_str()) ? true : false;
	if (0 != listOfArgValue[3].length())
		_pProgrammingIOTA_Data_TestData->WOF = atoi(listOfArgValue[3].c_str()) ? true : false;
}

void Ts_BravoProgrammingIOTA_DATA::Execute()
{
	uint32_t rc = 0;
	Syn_Exception Exception(0);

	_pProgrammingIOTA_Data_TestData->executed = true;

	unsigned int bincodesCounts = _pSynDutUtils->_pDutTestResult->list_bincodes.size();
	if (0 != bincodesCounts)
	{
		_pProgrammingIOTA_Data_TestData->pass = false;
		return;
	}
	...
	if (!_pSynDutUtils->Config_MT_Info.GetPatchInfo("iotawrite", IOTAWritePatch))
	{
		...
		Exception.SetDescription("BravoProgrammingIOTA_DATA::IOTAWritePatch not exists!");
		throw Exception;
		return;
	}
	...
	{
		Exception.SetError(ERROR_CONFIG_DATA_LOST);
		Exception.SetDescription("BravoProgrammingIOTA_DATA::IOTAWritePatch has no value!");
		throw Exception;
		return;
	}
	...
	if (0 != rc)
	{
		...
		Exception.SetDescription("BravoProgrammingIOTA_DATA::checkBLMode is failed!");
		throw Exception;
		return;
	}
	if (!InBootloaderMode)
	{
		...
		Exception.SetDescription("BravoProgrammingIOTA_DATA::InBootloaderMode is false!");
		throw Exception;
		return;
	}
	...
	{
		Exception.SetError(rc);
		Exception.SetDescription("BravoProgrammingIOTA_DATA::FpLoadPatch(iotawrite) is failed!");
		throw Exception;
		return;
	}

	_pProgrammingIOTA_Data_TestData->pass = true;
	...
	if (NULL != pCalibrateData)
	{
		...
		if (_pProgrammingIOTA_Data_TestData->LNA_BL)
		{
			...
			{
				Exception.SetError(rc);
				Exception.SetDescription("BravoProgrammingIOTA_DATA::FpIotawrite(LNA_BL) is failed!");
				throw Exception;
				return;
			}
		}

		if (_pProgrammingIOTA_Data_TestData->FW_BL)
		{
			...
			{
				Exception.SetError(rc);
				Exception.SetDescription("BravoProgrammingIOTA_DATA::FpIotawrite(FW_BL) is failed!");
				throw Exception;
				return;
			}
		}
	...
	{
		if (_pProgrammingIOTA_Data_TestData->LNA_BL || _pProgrammingIOTA_Data_TestData->FW_BL)
		{
			_pProgrammingIOTA_Data_TestData->pass = false;
		}
	}
	...
	{
		if (_pProgrammingIOTA_Data_TestData->WOF)
		{
			...
			{
				Exception.SetError(rc);
				Exception.SetDescription("BravoProgrammingIOTA_DATA::FpGetWOFRegisteris failed!");
				throw Exception;
				return;
			}
			...
			{
				Exception.SetError(rc);
				Exception.SetDescription("BravoProgrammingIOTA_DATA::FpIotawrite(gain) is failed!");
				throw Exception;
				return;
			}
			...
			{
				Exception.SetError(rc);
				Exception.SetDescription("BravoProgrammingIOTA_DATA::FpIotawrite(offset) is failed!");
				throw Exception;
				return;
			}
			...
			config_wof_thresholds.reduction = 0x8000;
			...
			{
				Exception.SetError(rc);
				Exception.SetDescription("BravoProgrammingIOTA_DATA::FpIotawrite(CONFIG_WOF_THRESHOLDS) is failed!");
				throw Exception;
				return;
			}
...
void Ts_BravoProgrammingIOTA_DATA::ProcessData()
{
	if (!_pProgrammingIOTA_Data_TestData->pass)
	{
		_pSynDutUtils->_pDutTestResult->list_bincodes.push_back("153");
	}
	_pSynDutUtils->_pDutTestResult->map_teststep_ispass.insert(map<string, string>::value_type(_pProgrammingIOTA_Data_TestData->data_name, _pProgrammingIOTA_Data_TestData->pass ? "Pass" : "Fail"));
}

void Ts_BravoProgrammingIOTA_DATA::CleanUp()
{
	CalculateTestTime(_pProgrammingIOTA_Data_TestData->test_time);

	StoreTestData(_pProgrammingIOTA_Data_TestData->data_name, static_cast<SynTestData*>(_pProgrammingIOTA_Data_TestData));
}